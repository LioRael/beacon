...
test("renders a versioned Apple Silicon formula", () => {
  const formula = renderFormula({ version: "0.2.0", sha256: "a".repeat(64) });

  assert.match(formula, /class Beacon < Formula/);
  assert.match(formula, /releases\/download\/v0\.2\.0\/beacon-v0\.2\.0-aarch64-apple-darwin\.tar\.gz/);
...
test("rejects unsafe formula inputs", () => {
  assert.throws(() => renderFormula({ version: "v0.2.0", sha256: "a".repeat(64) }), /version/);
  assert.throws(() => renderFormula({ version: "0.2.0", sha256: "nope" }), /SHA-256/);
});
