...
test("renders a versioned Apple Silicon formula", () => {
  const formula = renderFormula({ version: "0.3.0", sha256: "a".repeat(64) });

  assert.match(formula, /class Beacon < Formula/);
  assert.match(formula, /releases\/download\/v0\.3\.0\/beacon-v0\.3\.0-aarch64-apple-darwin\.tar\.gz/);
...
test("rejects unsafe formula inputs", () => {
  assert.throws(() => renderFormula({ version: "v0.3.0", sha256: "a".repeat(64) }), /version/);
  assert.throws(() => renderFormula({ version: "0.3.0", sha256: "nope" }), /SHA-256/);
});
...
test("release tags must match Rust and npm package versions", () => {
  assert.doesNotThrow(() =>
    verifyReleaseVersion({ tag: "v0.3.0", cargoVersion: "0.3.0", npmVersion: "0.3.0" }),
  );
  assert.throws(
    () => verifyReleaseVersion({ tag: "v0.4.0", cargoVersion: "0.3.0", npmVersion: "0.3.0" }),
    /does not match/,
  );
});
...
test("CI runs distribution, skill, and Formula validation", () => {
  ...
  assert.match(workflow, /verify-release-version\.mjs v0\.3\.0/);
});

test("README documents the two-layer model and release metadata at 0.3.0", () => {
  const readme = fs.readFileSync(new URL("../README.md", import.meta.url), "utf8");

  assert.match(readme, /# Beacon 0\.3/);
  assert.match(readme, /beacon-v0\.3\.0-aarch64-apple-darwin/);
